...
# OAuth callback: when OPENCLAW_DOMAIN and instance name are available,
# configure IronClaw to use the shared auth proxy for OAuth redirects.
...
# Enable Docker sandbox (DinD image has dockerd)
